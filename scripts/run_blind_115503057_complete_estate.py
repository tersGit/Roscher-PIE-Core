#!/usr/bin/env python3
"""Blind ranking of Property24 115503057 against carlswald_north_corrected_002.
...
LISTING_ID = "115503057"
LISTING_URL = (
    "https://www.property24.com/for-sale/summerset/midrand/gauteng/11682/115503057"
)
OUT_DIR = ROOT / "data/investigations/blind_115503057_complete_estate"
...
def _write_report(freeze: dict, marker: dict, adapter: dict, digest: str) -> Path:
    ...
    lines = [
        ...
        "Accuracy test on Hybrid extraction + FastSAM adapter + Corner Gate v1 + "
        "Pool Object Validation v1 + **Pool Inventory NO/UNKNOWN safety v1.1.0**. "
        "Scoring v2 weights unchanged. Water colour is not used in matching or scoring.",
        "",
        f"- **Freeze path:** `data/investigations/blind_115503057_complete_estate/freeze.json`",
        ...
        "prior advertisement / seller-social search. Prior `115503057` artefacts: **none found**. "
        "Photos downloaded fresh. Historical freeze trees were not modified.",
        "",
        "## B. Acquisition",
        "",
