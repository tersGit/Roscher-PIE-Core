#!/usr/bin/env python3
"""Blind ranking of Property24 117170887 against carlswald_north_corrected_002.

Accuracy test on PR #23 Hybrid extraction + PR #24 adapter + PR #26 Corner Gate
+ PR #27 Pool Object Validation, with unchanged Scoring v2.

This script is freeze-only. It does not look up street, stand, coordinates,
archives, or any other ground-truth identity. STOP after freeze.
"""
...
LISTING_URL = (
    "https://www.property24.com/for-sale/carlswald-north-estate/midrand/gauteng/12743/117170887"
)
OUT_DIR = ROOT / "data/investigations/blind_117170887_complete_estate"
...
def _write_report(freeze: dict, marker: dict, adapter: dict, digest: str) -> Path:
    ...
    lines = [
        ...
        "",
        f"- **Freeze path:** `data/investigations/blind_117170887_complete_estate/freeze.json`",
    ...
    for row in listing_pov.get("per_frame") or []:
        lines.append(
            f"| {row.get('media_id')} | {row.get('viewpoint')} | {row.get('source')} | "
            f"{row.get('scoring_ready')} | {row.get('pov_status')} | {row.get('object_role')} | "
            f"{row.get('principal_pool_candidate')} |"
        )
    lines.extend(
        [
            "",
            ...
            "",
            f"Gaps: 1–2={quality.get('gap_1_2')}, 1–5={quality.get('gap_1_5')}, 1–10={quality.get('gap_1_10')}, 1–20={quality.get('gap_1_20')}.",
            "",
            f"Genuine shape rows={quality.get('n_genuine_shape')}; shape_v2≥0.80={quality.get('n_shape_v2_ge_0_80')}; "
            f"Top1 evidence={quality.get('top1_evidence_pct')}% padding={quality.get('top1_padding_pct')}%.",
            "",
    ...
    for row, panel in zip(top5, panels or [None] * 5):
        lines.append(
            ...
            f"shape_v2={row.get('shape_v2')} POV={row.get('candidate_pov_status')} corner={row.get('parcel_corner')} "
            f"panel=`{panel}`"
        )
    lines.extend(
        [
...
def main() -> int:
    ...
    hits = list(prior.get("workspace_path_hits_excluded") or [])
    if hits or prior.get("frozen_hybrid_json_contains_listing"):
        raise SystemExit(f"listing {LISTING_ID} is not clean: {prior}")
    print("Phase 1 — acquire, Pool Gate, Corner Gate, POV overlay, Scoring v2 freeze")
    print("STOP after freeze. Ground-truth recovery is not part of this run.")
    print(f"  prior artefacts: none ({prior})")
    result = run_freeze(
    ...
    report_path = _write_report(freeze, marker, adapter, digest)
    print(
        "  adapter_accepted="
