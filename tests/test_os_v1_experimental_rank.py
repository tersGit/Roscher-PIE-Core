...
def test_neutral_fill_does_not_reward_rejected_over_confirmed():
    listing = _listing()
    rejected = os_object_features(
        listing,
        {
            "pool": {"status": "REJECTED", "geometry": {"present": True}},
            "building": {
                "status": "CONFIRMED",
                "geometry": {
                    "present": True,
    ...
    skip_rejected, _ = experimental_pure_os_score(rejected)
    skip_confirmed, _ = experimental_pure_os_score(confirmed)
    assert skip_rejected >= skip_confirmed  # skip-and-renormalize artefact

    neu_rejected, neu_r_contrib = experimental_pure_os_neutral_score(rejected)
    neu_confirmed, neu_c_contrib = experimental_pure_os_neutral_score(confirmed)
    assert neu_r_contrib["pool_presence"] == 0.07  # 0.5 * 0.14
    assert neu_confirmed > neu_rejected

    hy_rejected, _ = experimental_hybrid_neutral_score(
        rejected, aerial=None, video=None, exterior=None, stand_size=0.9
    )
    hy_confirmed, _ = experimental_hybrid_neutral_score(
        confirmed, aerial=None, video=None, exterior=None, stand_size=0.9
    )
    assert hy_confirmed > hy_rejected
